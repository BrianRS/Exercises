...
-- import Lecture1 (lastDigit, subString, strSum, lowerAndGreater)
...
subString start end str =
    let
        start' = max start 0
        end' = if end >= 0 then end + 1 else 0
    in
...
lowerAndGreater x xs =
    let
        lower = length (filter (< x) xs)
        greater = length (filter (> x) xs)
    in
        show x ++ " is greater than " ++ show lower ++ " elements and lower than " ++ show greater ++ " elements"
